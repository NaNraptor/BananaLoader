...
namespace Harmony
{
	...
	public class HarmonyInstance
	{
		...
		public void UnpatchAll(BananaBase Banana)
		{
			bool IDCheck(Patch patchInfo) => (patchInfo.patch.Module.Assembly == Banana.Assembly);
			var originals = GetPatchedMethods().ToList();
			foreach (var original in originals)
			{
				var info = GetPatchInfo(original);
		...
		public static void UnpatchAllBananaInstances(BananaBase Banana)
        {
			if (instancelist.Count > 0)
			{
				for (int i = 0; i < instancelist.Count; i++)
					instancelist[i].UnpatchAll(Banana);
				instancelist.Clear();
			}
		}

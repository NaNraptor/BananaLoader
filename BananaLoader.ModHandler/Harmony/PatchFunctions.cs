...
namespace Harmony
{
	public static class PatchFunctions
	{
		...
		private static IEnumerable<CodeInstruction> UnhollowerTranspiler(MethodBase method, IEnumerable<CodeInstruction> instructionsIn) {
			...
			if (!found) {
				BananaLogger.LogError("Harmony transpiler could not rewrite Unhollower method. Expect a stack overflow.");
				return instructions;
			}
		...
		private static DynamicMethod CreateIl2CppShim(DynamicMethod patch, MethodBase original)
		{
			...
			// BananaLogger.LogError("Exception in ...\n" + exception.ToString());
			...
			Emitter.Emit(il, OpCodes.Call, AccessTools.DeclaredMethod(typeof(BananaLogger), "LogError", new Type[] { typeof(string) }));

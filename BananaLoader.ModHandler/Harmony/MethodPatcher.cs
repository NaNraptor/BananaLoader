...
namespace Harmony
{
	public static class MethodPatcher
	{
		...
		{
			return CreatePatchedMethod(original, ("BananaLOADER_HARMONY_PATCH_" + BananaLoader.BuildInfo.Version), prefixes, postfixes, transpilers);
		}
		...
		public static DynamicMethod CreatePatchedMethod(MethodBase original, string harmonyInstanceID, List<MethodInfo> prefixes, List<MethodInfo> postfixes, List<MethodInfo> transpilers)
		{
			try
			{
				...
				bool isIl2Cpp = BananaLoader.UnhollowerSupport.IsGeneratedAssemblyType(original.DeclaringType);

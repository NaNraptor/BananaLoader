namespace BananaLoader
{
    public static class SceneHandler
    {
        ...
        public static void OnSceneLoad(int current_scene)
        {
            ...
            {
                if (BananaLoaderBase._IsBoneworks)
                {
            ...
            if (should_run)
            {
                ...
                if (!ShouldWait)
                    IsLoading = true;
                if (BananaLoaderBase._IsBoneworks)
                    BananaHandler.OnLevelIsLoading();
                else
        ...
        private static void CheckForSceneFinishedLoading()
        {
            if (IsLoading)
            {
                BananaHandler.OnLevelWasLoaded(LastSceneIndex);
        ...
        private static void CheckForSceneInitialized()
        {
            ...
            {
                BananaHandler.OnLevelWasInitialized(LastSceneIndex);

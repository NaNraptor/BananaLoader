...
namespace BananaLoader {
	internal class DependencyGraph<T> where T : BananaBase {
		...
		private DependencyGraph(IList<T> mods, Func<T, string> modNameGetter) {
			...
			foreach (Vertex modVertex in vertices) {
				...
				BananaOptionalDependenciesAttribute optionals = (BananaOptionalDependenciesAttribute) Attribute.GetCustomAttribute(modAssembly, typeof(BananaOptionalDependenciesAttribute));
			...
			if (modsWithMissingDeps.Count > 0) {
				...
				BananaLogger.LogWarning(BuildMissingDependencyMessage(modsWithMissingDeps));
			}
		}
		...
		private static bool TryLoad(AssemblyName assembly) {
			try {
				...
				BananaLogger.LogError("Loading mod dependency failed: " + ex);
				return false;
			}
		}
		...
		private static string BuildMissingDependencyMessage(IDictionary<string, IList<AssemblyName>> modsWithMissingDeps) {
			StringBuilder messageBuilder = new StringBuilder("Some mods are missing dependencies, which you may have to install.\n" +
				"If these are optional dependencies, mark them as optional using the BananaOptionalDependencies attribute.\n" +
				"This warning will turn into an error and mods with missing dependencies will not be loaded in the next version of BananaLoader.\n");
		...
		private void TopologicalSortInto(IList<T> loadedMods) {
			...
			if (loadedMods.Count + skippedMods < vertices.Length) {
				...
				BananaLogger.LogError(errorMessage.ToString());
			}
		}

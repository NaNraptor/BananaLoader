...
namespace BananaLoader
{
    public class BananaConsole
    {
        ...
        private static void Create()
        {
            Enabled = true;
            Allocate();
            Console.SetOut(new StreamWriter(new FileStream(new SafeFileHandle(BananaLogger.Native_GetConsoleOutputHandle(), false), FileAccess.Write)) { AutoFlush = true });

...
using BananaLoader.LightJson;
#pragma warning disable 0168

namespace BananaLoader.Installer
{
    static class Program
    {
        internal static bool ShouldCheckForUpdates = false;
        internal static string Title = "BananaLoader Installer";
        ...
        private static readonly string[] foldersToCleanUp = new string[] { "Logs", "BananaLoader" };
        ...
        static void Main()
        {
            ...
            else
            {
                ...
                ManualZipPath = (Directory.GetCurrentDirectory() + "\\BananaLoader.zip");
        ...
        static void Install_NoGUI()
        {
            try
            {
                ...
                {
                    MessageBox.Show("Installation failed; upload the created log to #Bananaloader-support on discord", Title);
            ...
            {
                MessageBox.Show("Failed to Get Version List; upload the created log to #Bananaloader-support on discord", Title);
                File.WriteAllText(Directory.GetCurrentDirectory() + $@"\MLInstaller_{DateTime.Now:yy-M-dd_HH-mm-ss.fff}.log", ex.ToString());
                Environment.Exit(-1);
            }
        }
        ...
        {
            SetDisplayText("Extracting BananaLoader.zip...");
            Cleanup(dirpath);
        ...
        internal static void Cleanup(string dirpath)
        {
            ...
            {
                DialogResult result = MessageBox.Show($"BananaLoader could not remove old files.{Environment.NewLine}Please close the game then click retry to try again.{Environment.NewLine}If issue persists please click cancel to dump logs.", Program.Title, MessageBoxButtons.RetryCancel, MessageBoxIcon.Error);
        ...
        private static void Install_Legacy_02(string dirpath, string selectedVersion)
        {
            ...
            if (is_02)
            {
                string AssemblyGenerator_Folder = Path.Combine(Path.Combine(dirpath, "BananaLoader"), "AssemblyGenerator");

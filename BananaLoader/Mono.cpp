...
void Mono::CreateDomain()
{
	if (Domain == NULL)
	{
		...
		mono_runtime_set_main_args(BananaLoader::CommandLineC, BananaLoader::CommandLineV);
		Domain = mono_jit_init("BananaLoader");
...
void Mono::FixDomainBaseDir()
{
	...
	if (!IsOldMono)
		mono_domain_set_config(Domain, BananaLoader::GamePath, "BananaLoader");
}

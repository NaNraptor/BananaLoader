...
void AssertionManager::ThrowInternalError(std::string msg)
{
	if (!Result)
	{
		...
		if (BananaLoader::DebugMode)
			MessageBox(NULL, msg.c_str(), "BananaLoader - INTERNAL FAILURE", MB_OK | MB_ICONERROR);
		else
			MessageBox(NULL, "Please Post your Latest Log File\non #internal-failure in the BananaLoader Discord!", "BananaLoader - INTERNAL FAILURE!", MB_OK | MB_ICONERROR);
	}
}

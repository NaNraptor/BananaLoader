...
void Logger::Log(const char* txt)
{
	...
	Console::Write("[");
	Console::Write("BananaLoader", ConsoleColor_Magenta);
	Console::Write("] ");
...
void Logger::LogWarning(const char* txt)
{
	if ((MaxWarnings <= 0) || (WarningCount < MaxWarnings))
	{
		...
		if (BananaLoader::DebugMode || !Console::HideWarnings)
		{
			Console::Write("[BananaLoader] ", ConsoleColor_Yellow);
...
void Logger::LogError(const char* txt)
{
	if ((MaxErrors <= 0) || (ErrorCount < MaxErrors))
	{
		...
		Console::Write("[BananaLoader] ", ConsoleColor_Red);
...
void Logger::LogBananaError(const char* namesection, const char* txt)
{
	if ((MaxErrors <= 0) || (ErrorCount < MaxErrors))
	{
		LogTimestamp(ConsoleColor_Yellow);
...
void Logger::LogBananaCompatibility(BananaLoader_Base::BananaCompatibility comp)
{
	LogTimestamp();
	Console::Write("[");
	Console::Write("BananaLoader", ConsoleColor_Magenta);
	...
	switch (comp)
	{
	case BananaLoader_Base::BananaCompatibility::UNIVERSAL:
		LogFile << "Universal";
		Console::WriteLine("Universal", ConsoleColor_Cyan);
		break;
	case BananaLoader_Base::BananaCompatibility::COMPATIBLE:
		LogFile << "Compatible";
		Console::WriteLine("Compatible", ConsoleColor_Green);
		break;
	case BananaLoader_Base::BananaCompatibility::NOATTRIBUTE:
		LogFile << "No BananaGameAttribute!";
		Console::WriteLine("No BananaGameAttribute!", ConsoleColor_Yellow);
		break;
	default:
